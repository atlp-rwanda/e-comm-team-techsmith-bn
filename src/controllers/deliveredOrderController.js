...
class delivery {
    static async deliverOrder({ params }, res) {
        try {
            ...
            const deliveredOrder = await Order.update({ status: 'delivered' }, {
                where: { id: params.id },
                returning: true,
                plain: true,
            });
    ...
    static async cancelDelivery({ params }, res) {
        try {
            ...
            const deliveredOrder = await Order.update({ status: 'paid' }, {
                where: { id: params.id },
                returning: true,
                plain: true,
            });
    ...
    static async deliveryMoving({ params }, res) {
        try {
            ...
            const deliveryMoving = await order.update({ status: 'onWay' }, {
                where: {
                    where: { id: params.id },
                    returning: true,
                    plain: true,
